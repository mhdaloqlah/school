...
function AdminStudentMark() {
  ...
  const { id } = useParams();
    ...
    const getMark = () => {
        axios
            .get(ApiMark + `?filter[student_id]=${id}&filter[year_id]=${year}&filter[term_id]=${term}&include=material.grade,teacher,student`)
            .then((res) => {
                if (res) {
    ...
    return (
        <Container style={{ paddingRight: "50px" }}>
            ...
            <div className="box">
                ...
                <div className="row">
                    <h3>درجات الطالب {student.first_name + ' '+  student.last_name}</h3>
                </div>
