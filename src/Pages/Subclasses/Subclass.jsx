...
function Subclass() {
  ...
  const Api = apilink + "subclass?include=students";
  ...
  const handleaddNewYear = async () => {
    try {
      ...
      axios.post(apilink + 'subclass', userData).then((response) => {
  ...
  const handleEditGrade = async () => {
    try {
      ...
      const response = await fetch(apilink + "subclass/" + gradeid, {
        method: "PUT",
        headers: {
  ...
  const handelDelete = async (id) => {
    ...
    // setIsLoading(true);
    try {
      const response = await fetch(apilink.concat("subclass/") + id, {
        method: "DELETE",
      });
      if (!response.ok) {
  ...
  const getGrade = (id) => {
    axios
      .get(apilink.concat("subclass/") + id)
      .then((res) => {
  ...
  return (
    <Container style={{ paddingRight: "50px" }}>
      <h1 style={{ marginTop: "25px" }}>الشعب</h1>
